package com.nwsuaf.werewolf.receiver;
...
public final class LocalBroadcastManager {
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                Log.i("LocalBroadcastManager", "Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            }

            ArrayList entries = (ArrayList) this.mActions.get(intent.getAction());
            if (entries != null) {
                if (debug) {
                    Log.i("LocalBroadcastManager", "Action list: " + entries);
                }
                ...
                for (i = 0; i < entries.size(); ++i) {
                    ...
                    if (debug) {
                        Log.i("LocalBroadcastManager", "Matching against filter " + receiver.filter);
                    }

                    if (receiver.broadcasting) {
                        if (debug) {
                            Log.i("LocalBroadcastManager", "  Filter\'s target already added");
                        }
                    } else {
                        int match = receiver.filter.match(action, type, scheme, data, categories, "LocalBroadcastManager");
                        if (match >= 0) {
                            if (debug) {
                                Log.i("LocalBroadcastManager", "  Filter matched!  match=0x" + Integer.toHexString(match));
                            }
                            ...
                            Log.i("LocalBroadcastManager", "  Filter did not match: " + reason);
                        }
                    }
                }
